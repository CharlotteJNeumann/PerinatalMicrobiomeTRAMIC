...
## Import the data
metabolomic_data <- read_csv("metabolomics_significant_mpost.csv")
...
genus_data <- read_csv("data_table_bacteria_urine_mpost.csv")
...
warnings()
#In cor(a[complete.cases(b), ], b[complete.cases(b), ],  ... :
...
ggsave("pearson correlation_mpost.png",
       width = 20,
       height = 25,
       units = c("cm"),
       dpi = 300)

write.csv(df,"pearson_correlation_mpost.csv")
