## this is the heatmap on how to plot the 13 significantly differentially abundant metabolites in a heatmap with the taxonomic data ##
...
## Import the data
metabolomic_data <- read_csv("metabolomics_significant.csv")
...
genus_data <- read_csv("data_table_bacteria_urine.csv")
...
genus_data <- read_csv("data_table_bacteria_genus.csv")
metabolomic_data <- read_csv("metabolomics_significant.csv")
...
if(adjustment==1){
  ...
  for(i in unique(df$Metabolite)){
    for(j in unique(df$Group)){
      sel<-df$Metabolite==i & df$Group==j
...
ggsave("pearson correlation_2.png",
       width = 20,
       height = 25,
       units = c("cm"),
       dpi = 300)

write.csv(df,"pearson_correlation_2.csv")
